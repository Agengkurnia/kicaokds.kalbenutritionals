    ...
    oTableBudget = $('#dtBudget').DataTable({
        ...
        aoColumnDefs: [
            ...
            },
            {
                ...
                mRender: function (data, type, full) {
                    return '     <div class="input-group"> ' +
                        ...
                        '       </div> ' +
                        '   </div>' +
                        '<div style="display:none;"> ' + full.TXT_DEPARTMENT + ' </div>'
                }
            },
            //{
            //    aTargets: [2],
            ...
            //},
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div > <input type="button" class="btn btn-info btnBudgetSubUmbrand" id="btnBudgetSubUmbrand" onclick="p_btnBudgetSubUmbrand_Click(this,' + full.intIndex + ')"  value="Sub Umbrand" >  </div>';
                }
            },
            {
                aTargets: [3],
...
function p_AddBudget() {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/ONO/AddRowBudget",
...
function p_DataToUIBudget(XXSHP_KDS_T_ONO_BGT) {
    
    oTableBudget.clear();
    for (var i = 0; i < XXSHP_KDS_T_ONO_BGT.length; i++) {
        XXSHP_KDS_T_ONO_BGT[i].intIndex = i;
        oTableBudget.row.add(XXSHP_KDS_T_ONO_BGT[i]);
    }
    oTableBudget.draw(false);

    var objDat = p_GetHiddenObject();
    objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT = XXSHP_KDS_T_ONO_BGT;
    p_SetHiddenObject(objDat);
}
...
function p_RefreshNumberBudget() {
    ...
    oTableBudget.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex].intIndex = d.intIndex;

        d.txtName = objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex].txtName;
        intRowIndex++;
...
function p_btnLOVDept_Click(objCaller, intIndex) {
    LOV = clsGlobal.generateLOV(MODULE_DEPARTMENT, "txtDepartment", "ONO");
}
...
function p_settxtMPPDolphine(txtValue) {
    ...
    oTableBudget.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        if (intRowIndex == intSelectedIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "/Transaction/ONO/AddPPDolphine",
                data: { intActivityIndex: intActivityIndex, intBudgetIndex: intSelectedIndex, txtPPDolphine: txtValue, data: $("#txtHiddenObject").val(), __RequestVerificationToken: $('#frmONO input[name=__RequestVerificationToken]').val() },
                datatype: "json",
                success: function (retDat) {
...
function p_btnLOVPPH_Click(objCaller, intIndex) {
    var intSelectedIndex = p_GetSelectedBudgetRow();
    var objDat = p_GetHiddenObject();
    LOV = clsGlobal.generateLOV(MODULE_PPH_SUPPLIER_ACTIVITY, "txtPPH", objDat.TXT_SUPPLIER_SITE_CODE + "|" + objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].TXT_ACTIVITYCODE);
}
...
function p_settxtPPH(txtValue1, txtValue2) {
    ...
    oTableBudget.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        if (intRowIndex == intSelectedIndex) {
            ...
            objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex].TXT_PPH = txtValue1;
            objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex].DEC_PPH = txtValue2;

            p_CalculateSubTotal(objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex]);

            d.DEC_SUBTOTAL = objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex].DEC_SUBTOTAL;
            d.DEC_PPH_AMOUNT = objDat.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[intRowIndex].DEC_PPH_AMOUNT;
...
//        if (objData.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[i].intIndex == intIndex) {
//            // Ketemu, mulai dari sini:
//            objData.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT[i].txtDescription = objCaller.value;
//            break;
//        }
//    }
//    p_SetHiddenObject(objData);
...
function p_ValidateBudgetInput() {
    ...
    var isValid = true;
    var objData = JSON.parse(p_UIToData());
    var bitAddendum = clsGlobal.ParseBooleanOracleToNET(objData.BIT_ADDENDUM);

    if (objData.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT.length > 0 && objData.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT.length < 2) {
        for (i = 0; i < objData.XXSHP_KDS_T_ONO_ACT[intActivityIndex].XXSHP_KDS_T_ONO_BGT.length; i++) {
            // Cari Index-nya.
        ...
        clsGlobal.setMessageWarning("Data tidak boleh kosong atau lebih dari satu!");
    }
