...
function p_InitForm() {
    ...
    INT_ONO_HDR_ID = $.getParameter("INT_ONO_HDR_ID");
...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/ONO/CheckBudget",
        data: {
            INT_ONO_HDR_ID: INT_ONO_HDR_ID.toString(),
...
function p_InitiateDetail() {
    ...
    oTableDetail = $('#dtCheckBudget').DataTable({
        "bPaginate": true,
        "bSort": false,
        searching: false,
        "iDisplayLength": 5,
        ...
        aoColumnDefs: [
            {
                ...
                mRender: function (data, type, full) {
                    return '<div id="lblNo"> ' + (full.NO) + ' </div>';
                }
            },
            {
                aTargets: [1],
                mRender: function (data, type, full) {
                    return '<div id="lblNo"> ' + full.TXT_SUBUMBRAND + ' </div>';
                }
            },
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div id="lblNo" style="text-align:right;"> ' + "Rp. " + clsGlobal.parseToRupiah(full.AmountSubUmbrandBudgetBesar) + ' </div>';
                }
            },
            {
                aTargets: [3],
                mRender: function (data, type, full) {
                    return '<div id="lblNo" style="text-align:right;"> ' + "Rp. " +  clsGlobal.parseToRupiah(full.AmountSubUmbrandONOBudget) + ' </div>';
                }
            },
            {
                aTargets: [4],
                mRender: function (data, type, full) {
                    return '<div id="lblNo" style="text-align:right;"> ' + "Rp. " +  clsGlobal.parseToRupiah(full.EstimateEndingBalance) + ' </div>';
                }
            }
        ]
    });

    $("#dtCheckBudget").css("width", "100%");
    $('#dtCheckBudget tbody').on('click', 'tr', function () {
