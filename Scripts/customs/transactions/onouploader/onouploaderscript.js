...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/ONOUploader/InitiateData",
...
function p_txtDocNo_TextChanged(ID) {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/ONOUploader/GetData",
...
function p_processData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/ONOUploader/ProcessData",
...
//function p_PopulateGroupAccount() {
//    clsGlobal.showLoading();
//    
//    $.ajax({
//        type: "POST",
//        url: "/Transaction/ONOUploader/PopulateGroupAccount",
//        data: { __RequestVerificationToken: $('#frmIndex input[name=__RequestVerificationToken]').val() },
//        datatype: "json",
//        success: function (retDat) {
//            if (retDat.bitSuccess == true) {
//                
//                if (retDat.objData != undefined) {
//                    $('#dllGroupAccount').empty();
//                    $('#dllGroupAccount').append($('<option>').text("-").prop('value', ""));
//                    for (var i = 0; i < retDat.objData.length; i++) {
//                        $('#dllGroupAccount').append($('<option>').text(retDat.objData[i]).prop('value', retDat.objData[i]));
//                    }

//                    //if (txtValue != "") {
//                    //    $("#dllGroupAccount").val(txtValue);
//                    //}
//                }
//            } else {
//                clsGlobal.getAlert(retDat.txtMessage);
    ...
    oTableDetail = $('#dtDetail').DataTable({
        ...
        aoColumnDefs: [
            ...
            },
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailParentDocNoValue"> ' + clsGlobal.parseToString(full.TXT_REFDOCNO) + ' </div>';
                }
            },
            {
                aTargets: [3],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailReffDocNoValue"> ' + clsGlobal.parseToString(full.TXT_GROUP_ACCOUNT) + ' </div>';
                }
            },
            {
                aTargets: [4],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailOutletValue"> ' + clsGlobal.parseToString(full.TXT_BUDGET_TYPE) + ' </div>';
                }
            },
            {
                aTargets: [5],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailPaymentValue"> ' + clsGlobal.parseToDateTimeFromJSON(full.DTM_POSTING, clsDateFormat) + ' </div>';
                }
            },
            {
                aTargets: [6],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailRemarkValue"> ' + clsGlobal.parseToString(full.TXT_PROGRAM_DESC) + ' </div>';
                }
            },
            {
                aTargets: [7],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailMonthlyPromoValue"> ' + clsGlobal.parseToString(full.TXT_MEKANISME) + ' </div>';
                }
            },
            {
                aTargets: [8],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailNationalValue"> ' + clsGlobal.parseToString(full.TXT_REMARK) + ' </div>';
                }
            },
            {
                aTargets: [9],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailActivityValue"> ' + clsGlobal.parseToString(full.TXT_ACTIVITYCODE) + ' </div>';
                }
            },
            {
                aTargets: [10],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailProgramDescValue"> ' + clsGlobal.parseToDateTimeFromJSON(full.DTM_PERIOD_FROM, clsDateFormat) + ' </div>';
                }
            },
            {
                aTargets: [11],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailPeriodFromValue"> ' + clsGlobal.parseToDateTimeFromJSON(full.DTM_PERIOD_TO, clsDateFormat) + ' </div>';
                }
            },
            {
                aTargets: [12],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailPeriodToValue"> ' + clsGlobal.parseToString(full.TXT_DEPARTMENT) + ' </div>';
                }
            },
            //{
            //    aTargets: [13],
            //    mRender: function (data, type, full) {
            //        return '<div id="lblDetailNeedByDateValue"> ' + clsGlobal.parseToString(full.TXT_REFDOCNO_DOLPHINE) + ' </div>';
            //    }
            //},
            {
                aTargets: [13],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDepartmentValue"> ' + clsGlobal.parseToString(full.TXT_SUBUMBRAND) + ' </div>';
                }
            },
            {
                aTargets: [14],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailMPPDocNoValue"> ' + full.DEC_AMOUNT + ' </div>';
                }
            },
            {
                aTargets: [15],
...
function p_Upload() {
    clsGlobal.getConfirmation("Upload ONO?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Transaction/ONOUploader/Upload",
                processData: false,
                contentType: false,
                data: data,
                success: function (retDat) {
...
function p_Download() {
    clsGlobal.getConfirmation("Download Template UploadONO?", function (result) {
        if (result == true) {
            clsGlobal.showLoading();
            $.ajax({
                type: "POST",
                url: "/Transaction/ONOUploader/Download",
...
function p_DownloadResult() {
    clsGlobal.getConfirmation("Download Result?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Transaction/ONOUploader/DownloadResult",
                data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmIndexUploader input[name=__RequestVerificationToken]').val() },
                datatype: "json",
                success: function (url) {
                    window.location = url;
