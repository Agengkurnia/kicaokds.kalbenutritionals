...
function p_DataToUI(objDat) {
    ...
    p_DataToUISupplier(objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP);
        ...
        success: function (retDat) {
            ...
            if (retDat.bitSuccess == true) {
                if (retDat.objData != undefined) {
                    ...
                    p_DataToUISupplier(retDat.objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP);
...
function p_DataToUISupplier(XXSHP_KDS_T_MKPP_SUP) {
    
    oTableSupplier.clear();
    for (var i = 0; i < XXSHP_KDS_T_MKPP_SUP.length; i++) {
        XXSHP_KDS_T_MKPP_SUP[i].intIndex = i;
        oTableSupplier.row.add(XXSHP_KDS_T_MKPP_SUP[i]);
    }
    oTableSupplier.draw(false);

    var objDat = p_GetHiddenObject();
    objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP = XXSHP_KDS_T_MKPP_SUP;
    p_SetHiddenObject(objDat);
}
...
function p_RefreshNumberSupplier() {
    ...
    oTableSupplier.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].intIndex = d.intIndex;
        
        d.TXT_ACCOUNT_SITE = objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].TXT_ACCOUNT_SITE;
        d.TXT_ACCOUNT_NAME = objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].TXT_ACCOUNT_NAME; 
        d.BIT_ACTIVE = objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].BIT_ACTIVE;
...
function p_settxtSupplier(txtValue1, txtValue2) {
    ...
    oTableSupplier.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        if (intRowIndex == intSelectedIndex) {
            ...
            objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].TXT_ACCOUNT_SITE = txtValue1;
            objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].TXT_ACCOUNT_NAME = txtValue2;
            d.BIT_ACTIVE = objDat.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[intRowIndex].BIT_ACTIVE;
...
function p_chkSupplierActive_Changed(objCaller, intIndex) {
    ...
    for (i = 0; i < objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP.length; i++) {
        // Cari Index-nya.
        if (objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[i].intIndex == intIndex) {
            // Ketemu, mulai dari sini:
            objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[i].BIT_ACTIVE = clsGlobal.ParseBooleanNETToOracle(objCaller.checked);
            ...
            //        d.TXT_ACCOUNT_SITE = objDat.XXSHP_KCO_T_MKPP_ACT[intActivityIndex].XXSHP_KCO_T_MKPP_SUP[intRowIndex].TXT_ACCOUNT_SITE;
            //        d.TXT_ACCOUNT_NAME = objDat.XXSHP_KCO_T_MKPP_ACT[intActivityIndex].XXSHP_KCO_T_MKPP_SUP[intRowIndex].TXT_ACCOUNT_NAME; 
            //        d.BIT_ACTIVE = objDat.XXSHP_KCO_T_MKPP_ACT[intActivityIndex].XXSHP_KCO_T_MKPP_SUP[intRowIndex].BIT_ACTIVE;
...
function p_btnSupplierDelete_Click(objCaller, intIndex) {
    ...
    for (i = 0; i < objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP.length; i++) {
        ...
        if (objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[i].intIndex == intIndex) {
            // Ketemu, mulai dari sini:
            // Remove from list.
            objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP.splice(i, 1);
...
function p_ValidateSupplierMKPP() {
    
    // Buat pengecekan supplier MKPP jika tidak ada yang active tidak bisa di OK
    var isValidSupplier = false
    var objData = JSON.parse(p_UIToData());
    for (i = 0; i < objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP.length; i++) {
        if (objData.XXSHP_KDS_T_MKPP_ACT[intActivityIndex].XXSHP_KDS_T_MKPP_SUP[i].BIT_ACTIVE == "Y") {
            // Ketemu, mulai dari sini:
    ...
    if (isValidSupplier) {
        ...
        clsGlobal.showAlert("Supplier MKPP tidak ada yang active !");
    }
}
