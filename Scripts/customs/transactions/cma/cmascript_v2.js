...
function setChooseLOV(txtValue) {
    ...
    switch (arr[0]) {
        ...
        case "txtRefDocNo":
            ...
            break;
        case "txtRefDocNoMkpp":
            //p_settxtRefDocNoMkpp(arr[1], arr[5], arr[7], arr[8], arr[9], arr[10], arr[4])
            debugger
            p_settxtRefDocNoMkpp(arr[1], arr[2], arr[3], arr[5], arr[6], arr[8], arr[9], arr[10])
            break;
...
function p_DataToUI(objData) {
    ...
    p_DataToUIMatchMkpp(objData.XXSHP_KDS_T_CMA_MAT_MKPP);
        ...
        success: function (retDat) {
            ...
            if (retDat.bitSuccess == true) {
                ...
                if (retDat.objData != undefined) {
                    ...
                    //setiap diganti outlet nya di bersihkan data detail nya
                    p_DeleteAllDataDetail();
                }
            }
            clsGlobal.hideLoading();
            $("#txtGUID").val(retDat.txtGUID);
    ...
    for (i = 0; i < objData.XXSHP_KDS_T_CMA_DTL.length; i++) {
        ...
        if (objData.XXSHP_KDS_T_CMA_DTL[i].bitSelected == true) {
            ...
            if (clsGlobal.ParseBooleanOracleToNET(objData.XXSHP_KDS_T_CMA_DTL[i].BIT_PENDING) != true) {
                ...
                //decTotalKlaim += objData.XXSHP_KDS_T_CMA_DTL[i].DEC_SUBTOTAL;
                decTotalKlaim += objData.XXSHP_KDS_T_CMA_DTL[i].DEC_DPP;
            }
        }
    }
    //masukin ke hidden object value nya biar kekirim di backend
    objData.DEC_TOTAL_KLAIM = decTotalKlaim;
    p_SetHiddenObject(objData);
...
$('#btnAddMatch').on('click', function () {
    try {
        var objDat = p_GetHiddenObject();
        if (objDat.XXSHP_KDS_T_CMA_MAT_MKPP.length == 0) {
            p_AddRowMatch();
        } else {
            alert('Tidak dapat matching KPP !! Harap hapus data matching MKPP terlebih dahulu...');
        }
    } catch (ex) {
        clsGlobal.showAlert(ex);
    }
...
function p_AddRowMatchKPP() {
    ...
    LOV = clsGlobal.generateLOV(MODULE_KPP_MATCHING_KPP, "KPPMatch", $("#txtGroupAccount").val() + "|" + $("#txtOutlet").val() + "|" + txtActivityCode + "|" + $("#txtSupplierID").val() + "|" + objDat.TXT_SOURCE_DOC);
...
function p_btnLOVRefDocNo_Click(objCaller, intIndex) {
    ...
    LOV = clsGlobal.generateLOV(MODULE_KPP_MATCHING, "txtRefDocNo", $("#txtGroupAccount").val() + "|" + $("#txtOutlet").val() + "|" + txtActivityCode + "|" + $("#txtSupplierID").val() + "|" + objDat.TXT_SOURCE_DOC);
}
...
function p_txtMatchAmount_Changed(objCaller, intIndex) {
    ...
    for (i = 0; i < objData.XXSHP_KDS_T_CMA_MAT.length; i++) {
        ...
        if (objData.XXSHP_KDS_T_CMA_MAT[i].intIndex == intIndex) {
            ...
            //if (clsGlobal.parseToDecimal(objCaller.value) > objData.XXSHP_KDS_T_CMA_MAT[i].DEC_AMOUNT_AVAILABLE) {
            //    objData.XXSHP_KDS_T_CMA_MAT[i].DEC_MATCHING = 0;
...
function p_CalculateTotalMatching() {
    ...
    //masukin ke hidden object value nya biar kekirim di backend
    objData.DEC_TOTAL_MATCHING = decTotalMatching;
    p_SetHiddenObject(objData);
...
//  Match MKPP
// ====================================================
//ALB - [20220711] Enhancement MKPP Matching - Start

function p_DataToUIMatchMkpp(XXSHP_KDS_T_CMA_MAT_MKPP) {
    
    oTableMatchMkpp.clear();
    for (var i = 0; i < XXSHP_KDS_T_CMA_MAT_MKPP.length; i++) {
        XXSHP_KDS_T_CMA_MAT_MKPP[i].intIndex = i;
        oTableMatchMkpp.row.add(XXSHP_KDS_T_CMA_MAT_MKPP[i]);
    }
    oTableMatchMkpp.draw(false);

    var objDat = p_GetHiddenObject();
    objDat.XXSHP_KDS_T_CMA_MAT_MKPP = XXSHP_KDS_T_CMA_MAT_MKPP;
    p_SetHiddenObject(objDat);
}

$('#btnAddRowMKPP').on('click', function () {
    try {
        //Check apakah Match KPP nya ada? kalo ada ga bisa untuk match MKPP
        var objDat = p_GetHiddenObject();
        if (objDat.XXSHP_KDS_T_CMA_MAT.length == 0) {
            p_AddRowMatchMkpp();
        } else {
            alert('Tidak dapat matching MKPP !! Harap hapus data matching KPP terlebih dahulu...');
        }
    } catch (ex) {
        clsGlobal.showAlert(ex);
    }
...
function p_AddRowMatchMkpp() {
    
    clsGlobal.showLoading();
    $.ajax({
        type: "POST",
        url: "/Transaction/CMA/AddRowMkpp",
        data: { data: $("#txtHiddenObject").val(), __RequestVerificationToken: $('#frmCMA input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
            ...
            if (retDat.bitSuccess == true) {
                if (retDat.objData != undefined) {
                    ...
                    p_DataToUIMatchMkpp(retDat.objData.XXSHP_KDS_T_CMA_MAT_MKPP);
                    oTableMatchMkpp.page('last').draw(false);
                }
            } else {
                clsGlobal.getAlert(retDat.txtMessage);
            }
...
$('#btnAddMatchMKPP').on('click', function () {
    try {
        var objDat = p_GetHiddenObject();
        if (objDat.XXSHP_KDS_T_CMA_MAT.length == 0) {
            p_AddRowMatchMKPP();
...
function p_AddRowMatchMKPP() {
    ...
    for (var i = 0; i < objDat.XXSHP_KDS_T_CMA_DTL.length; i++) {
        if (objDat.XXSHP_KDS_T_CMA_DTL[i].bitSelected == true) {
            txtActivityCode = objDat.XXSHP_KDS_T_CMA_DTL[i].TXT_ACTIVITYCODE;
            txtInvoiceNo = objDat.XXSHP_KDS_T_CMA_DTL[i].TXT_INVOICE_NO;
            txtKlaimDtlId = objDat.XXSHP_KDS_T_CMA_DTL[i].INT_KLAIM_DTL_ID;
            break;
        }
    }
    LOV = clsGlobal.generateLOV(MODULE_MKPP_MATCHING_MKPP, "MKPPMatch", $("#txtGroupAccount").val() + "|" + $("#txtOutlet").val() + "|" + txtActivityCode + "|" + $("#txtSupplierID").val() + "||" + txtInvoiceNo + "|" + txtKlaimDtlId);


}

function p_setMKPPMatch(txtValue) {
    //debugger
    clsGlobal.showLoading();
    $.ajax({
        type: "POST",
        url: "/Transaction/CMA/AddRowMatchMKPP",
        data: { txtMKPPMatch: txtValue, data: $("#txtHiddenObject").val(), __RequestVerificationToken: $('#frmCMA input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_InitiateMatchMKPP() {
    // Format datatable  
    //
    oTableMatchMkpp = $('#dtMatchMkpp').DataTable({
        "bPaginate": true,
        "bSort": false,
        searching: true,
        "aLengthMenu": [[5, 10, 25, 50, 75, 100], [5, 10, 25, 50, 75, 100]],
        ...
        aoColumnDefs: [
            {
                aTargets: [0],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppNoValue"> ' + (full.intIndex + 1) + ' </div>';
                }
            },
            {
                aTargets: [1],
                mRender: function (data, type, full) {
                    return '     <div class="input-group"> ' +
                        '       <div class="input-group-btn"> ' +
                        '           <button type="button" class="btn btn-danger btnLOVRefDocNoMkpp" id="btnLOVRefDocNoMkpp" onclick="p_btnLOVRefDocNoMkpp_Click(this,' + full.intIndex + ')"> <i class="fa fa-search"></i></button> ' +
                        '       </div> ' +
                        '       <input type="text" class="form-control txtRefDocNoMkpp" id="txtRefDocNoMkpp" onchange="p_txtRefDocNoMkpp_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_REFDOCNO + '" disabled> ' +
                        '   </div>';
                }
            },
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppActivity"> ' + clsGlobal.parseToString(full.TXT_ACTIVITY_CODE) + ' </div>';
                }
            },
            {
                aTargets: [3],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppPeriodFrom"> ' + clsGlobal.parseToString(full.DTM_PERIOD) + ' </div>';
                }
            },
            {
                aTargets: [4],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppPeriodTo"> ' + clsGlobal.parseToString(full.TXT_BUDGET_TYPE) + ' </div>';
                }
            },
            {
                aTargets: [5],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppSubbrand"> ' + clsGlobal.parseToString(full.TXT_SUBBRAND) + ' </div>';
                }
            },
            {
                aTargets: [6],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppAmountAvailable"> ' + clsGlobal.FormatMoney(full.DEC_AMOUNT_AVAILABLE, 0) + ' </div>';
                }
            },
            {
                aTargets: [7],
                mRender: function (data, type, full) {
                    return '<div id="lblMatchMkppRatio"> ' + clsGlobal.FormatMoney(full.DEC_RATIO, 4) + ' </div>';
                }
            },
            {
                aTargets: [8],
                mRender: function (data, type, full) {
                    return '<div >  <input type="text" class="form-control txtMatchMkppAmount autonumeric text-right" id="txtMatchMkppAmount' + full.intIndex + '"  onchange="p_txtMatchMkppAmount_Changed(this,' + full.intIndex + ')"  value="' + full.DEC_MATCHING + '" >  </div>';
                }
            },
            {
                aTargets: [9],
                mRender: function (data, type, full) {
                    return '<div > <input type="button" class="btn btn-warning btnMatchMkppDelete" id="btnMatchMkppDelete" onclick="p_btnMatchMkppDelete_Click(this,' + full.intIndex + ')"  value="Delete" >  </div>';
                }
            }
        ],
        "fnDrawCallback": function (oSettings) {
    ...
    $("#dtMatchMkpp").css("width", "100%");
    $('#dtMatchMkpp tbody').on('click', 'tr', function () {
        if (!$(this).hasClass('selected')) {
            oTableMatchMkpp.$('tr.selected').removeClass('selected');
            $(this).addClass('selected');
        }
    });

    $('#dtMatchMkpp').on('page.dt', function () {
        ////alert('go');
        p_GenerateAutoNumeric();
        //var info = table.page.info();
        //$('#pageInfo').html('Showing page: ' + info.page + ' of ' + info.pages);
...
function p_GetSelectedMKPPMatchRow() {
    var intIndex = clsGlobal.parseToInteger(oTableMatchMkpp.$('tr.selected').find("#lblMatchMkppNoValue").html()) - 1;
    return intIndex;
}

function p_btnLOVRefDocNoMkpp_Click(objCaller, intIndex) {
    // LOV = clsGlobal.generateLOV(MODULE_KPP, "txtRefDocNo");
    var objDat = p_GetHiddenObject();
    var txtActivityCode = "";
    var txtInvoiceNo = "";
    ...
    debugger
    LOV = clsGlobal.generateLOV(MODULE_MKPP_MATCHING, "txtRefDocNoMkpp", $("#txtGroupAccount").val() + "|" + $("#txtOutlet").val() + "|" + txtActivityCode + "|" + $("#txtSupplierID").val() + "|" + objDat.TXT_SOURCE_DOC + "|" + txtInvoiceNo + "|" + txtKlaimDtlId);
}

function p_settxtRefDocNoMkpp(txtValue, txtValue2, txtValue3, txtValue5, txtValue6, txtValue8, txtValue9, txtValue10) {
    //txtValue = No MKPP Dokumen
    //console.log("txtValue:" + txtValue );

    //txtValue5 = Aktifitas
    //console.log("txtValue5:" + txtValue5);

    //txtValue7 = Total Amount
    //console.log("txtValue7:" + txtValue7);

    //txtValue8 = Amount Available
    //console.log("txtValue8:" + txtValue8);

    //txtValue9 = ga kepake
    //console.log("txtValue9:" + txtValue9);

    //txtValue10 = Ref MKPP RKP ID
    //console.log("txtValue10:" + txtValue10);

    //txtValue4 = Sub Brand
    //console.log("txtValue4:" + txtValue4);

    try {
        var isValid = false;
        p_ValidateMKPPAttachment(txtValue, function (callback) {
            if (callback == true) {
                isValid = true;
            }
            else {
        ...
        if (isValid) {
            var intSelectedIndex = p_GetSelectedMKPPMatchRow();
            var intRowIndex = 0;
            var objDat = p_GetHiddenObject();
            oTableMatchMkpp.rows().every(function (rowIdx, tableLoop, rowLoop) {
                ...
                if (intRowIndex == intSelectedIndex) {
                    ...
                    d.TXT_REFDOCNO = txtValue;
                    //d.DTM_PERIOD_FROM = txtValue2;
                    //d.DTM_PERIOD_TO = txtValue3;
                    d.DTM_PERIOD = txtValue2;
                    d.TXT_BUDGET_TYPE = txtValue3;
                    d.DEC_AMOUNT_AVAILABLE = clsGlobal.parseToDecimal(txtValue9);
                    d.INT_KPP_RKP_ID = clsGlobal.parseToDecimal(txtValue10);
                    d.TXT_SUBBRAND = txtValue5;
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_REFDOCNO = txtValue;
                    //objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DTM_PERIOD_FROM = txtValue2;
                    //objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DTM_PERIOD_TO = txtValue3; dicomment karena dijadiin 1 dibawah
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DTM_PERIOD = txtValue2;
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_BUDGET_TYPE = txtValue3;
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DEC_AMOUNT_AVAILABLE = d.DEC_AMOUNT_AVAILABLE;
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].INT_KPP_RKP_ID = d.INT_KPP_RKP_ID;
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_ACTIVITY_CODE = txtValue6;
                    d.TXT_ACTIVITY_CODE = txtValue6;
                    objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_SUBBRAND = txtValue5;
            ...
            p_SetHiddenObject(objDat);
            p_RefreshDataRow_MKPP();
        }

    } catch (e) {
        clsGlobal.getAlert(e.message);
    }

}


function p_txtMatchMkppAmount_Changed(objCaller, intIndex) {
    
    // Parse dari HiddenObject->JSON
    var objData = JSON.parse(p_UIToData());
    for (i = 0; i < objData.XXSHP_KDS_T_CMA_MAT_MKPP.length; i++) {
        // Cari Index-nya.
        if (objData.XXSHP_KDS_T_CMA_MAT_MKPP[i].intIndex == intIndex) {
            // Ketemu, mulai dari sini:
            ...
            //    objData.XXSHP_KDS_T_CMA_MAT[i].DEC_MATCHING = clsGlobal.parseToDecimal(objCaller.value);
            //}
            objData.XXSHP_KDS_T_CMA_MAT_MKPP[i].DEC_MATCHING = clsGlobal.parseToDecimal(objCaller.value);
    ...
    p_SetHiddenObject(objData);
    p_CalculateTotalMatchingMKPP();
    p_RefreshMKPPMatchRow();

}


function p_CalculateTotalMatchingMKPP() {
    
    // Parse dari HiddenObject->JSON
    var objData = JSON.parse(p_UIToData());
    var decTotalMatchingMKPP = 0;
    for (i = 0; i < objData.XXSHP_KDS_T_CMA_MAT_MKPP.length; i++) {
        // Cari Index-nya.
        decTotalMatchingMKPP += objData.XXSHP_KDS_T_CMA_MAT_MKPP[i].DEC_MATCHING;
    }
    //masukin ke hidden object value nya biar kekirim di backend
    objData.DEC_TOTAL_MATCHING = decTotalMatchingMKPP;
    p_SetHiddenObject(objData);

    $("#lblTotalMatchingValue").html(clsGlobal.FormatMoney(decTotalMatchingMKPP, 0));
}
function p_btnMatchMkppDelete_Click(objCaller, intIndex) {
    ...
    for (i = 0; i < objData.XXSHP_KDS_T_CMA_MAT_MKPP.length; i++) {
        ...
        if (objData.XXSHP_KDS_T_CMA_MAT_MKPP[i].intIndex == intIndex) {
            // Ketemu, mulai dari sini:
            // Remove from list.
            objData.XXSHP_KDS_T_CMA_MAT_MKPP.splice(i, 1);

            //var row = oTableMatch.row($(this).parents('tr'));
            //var rowNode = row.node();
            //row.remove().draw();
            oTableMatchMkpp.row(i).remove().draw(false);
            break;
        }
    }
    p_SetHiddenObject(objData);
    p_CalculateTotalMatchingMKPP();
    p_RefreshNumberMatchMkpp();
}

function p_RefreshNumberMatchMkpp() {
    ...
    oTableMatchMkpp.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        var d = this.data();
        d.intIndex = intRowIndex; // update data source for the row
        objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].intIndex = d.intIndex;

        d.TXT_REFDOCNO = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_REFDOCNO;
        d.DEC_MATCHING = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DEC_MATCHING;
        d.TXT_SUBBRAND = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_SUBBRAND;
...
function p_RefreshMKPPMatchRow() {
    var intSelectedIndex = p_GetSelectedMKPPMatchRow();
    var intRowIndex = 0;
    var objDat = p_GetHiddenObject();
    oTableMatchMkpp.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        if (intRowIndex == intSelectedIndex) {
            ...
            d.TXT_REFDOCNO = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_REFDOCNO;
            d.DEC_AMOUNT_AVAILABLE = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DEC_AMOUNT_AVAILABLE;
            d.DEC_MATCHING = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].DEC_MATCHING;
            d.TXT_SUBBRAND = objDat.XXSHP_KDS_T_CMA_MAT_MKPP[intRowIndex].TXT_SUBBRAND;
...
function p_RefreshDataRow_MKPP() {
    var intSelectedIndex = p_GetSelectedActivityRow();
    var intRowIndex = 0;
    var objDat = p_GetHiddenObject();
    oTableActivity.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        var d = this.data();
        if (intRowIndex == intSelectedIndex) {
            d.intIndex = intRowIndex; // update data source for the row sampesini
...
function p_ValidateMKPPAttachment(txtValue, callback) {
    $.ajax({
        type: "POST",
        url: "/Transaction/CMA/AddRowMatchMKPP",
        data: { txtMKPPMatch: txtValue, data: $("#txtHiddenObject").val(), __RequestVerificationToken: $('#frmCMA input[name=__RequestVerificationToken]').val() },
...
function p_DeleteAllDataDetail() {
    
    //delete secara hidden object
    var objData = JSON.parse(p_UIToData());
    var originalTotalRowMKPP = objData.XXSHP_KDS_T_CMA_MAT_MKPP.length;
    for (i = 0; i < originalTotalRowMKPP; i++) {
        objData.XXSHP_KDS_T_CMA_MAT_MKPP.splice(0, 1);
    }
    p_SetHiddenObject(objData);

    //delete secara ui table
    oTableMatchMkpp.clear();
    oTableMatchMkpp.draw(false);
    p_CalculateTotalMatchingMKPP();
    ...
    //delete secara hidden object
    var objData = JSON.parse(p_UIToData());
    var originalTotalRow = objData.XXSHP_KDS_T_CMA_MAT.length;
    for (i = 0; i < originalTotalRow; i++) {
        objData.XXSHP_KDS_T_CMA_MAT.splice(0, 1);
    }
    p_SetHiddenObject(objData);
...
//MPR - [20240102] Enhancement MKPP Matching - End || update mengikuti Kicao MT by MPR
