...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/MKPPCloseUploader/InitiateData",
...
function p_txtDocNo_TextChanged(ID) {
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/MKPPCloseUploader/GetData", 
...
function p_processData() { 
    ...
    $.ajax({
        type: "POST",
        url: "/Transaction/MKPPCloseUploader/ProcessData",
...
function p_DataToUIDetail(XXSHP_KDS_T_MKPC_UPL) {
    
    oTableDetail.clear();
    for (var i = 0; i < XXSHP_KDS_T_MKPC_UPL.length; i++) {
        XXSHP_KDS_T_MKPC_UPL[i].intIndex = i;
        oTableDetail.row.add(XXSHP_KDS_T_MKPC_UPL[i]);
    }
    oTableDetail.draw(false);

    var objDat = p_GetHiddenObject();
    objDat = XXSHP_KDS_T_MKPC_UPL;
...
function p_Upload() {
    clsGlobal.getConfirmation("Upload MKPP Close?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Transaction/MKPPCloseUploader/Upload",
                processData: false,
                contentType: false,
                data: data,
                success: function (retDat) {
...
function p_CheckCanProcess(XXSHP_KDS_T_MKPC_UPL) {
    $("#btnProcess").hide();
    
    var bitFlag = true;
    if (XXSHP_KDS_T_MKPC_UPL.length == 0) {
        bitFlag = false;
    }
    for (var i = 0; i < XXSHP_KDS_T_MKPC_UPL.length; i++) {
        if (XXSHP_KDS_T_MKPC_UPL[i].TXT_STATUS == "E" || XXSHP_KDS_T_MKPC_UPL[i].TXT_STATUS == "S") {
            bitFlag = false;
        }       
    } 
    if (bitFlag == true) {
...
function p_Download() {
    clsGlobal.getConfirmation("Download Template UploadMKPP Close?", function (result) {
        if (result == true) {
            clsGlobal.showLoading();
            $.ajax({
                type: "POST",
                url: "/Transaction/MKPPCloseUploader/Download",
...
function p_DownloadResult() {
    clsGlobal.getConfirmation("Download Result?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Transaction/MKPPCloseUploader/DownloadResult",
                data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmIndexUploader input[name=__RequestVerificationToken]').val() },
                datatype: "json",
                success: function (url) {
                    window.location = url;
