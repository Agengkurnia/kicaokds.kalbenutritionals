...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/BudgetAdditionalV2/InitiateData",
        data: { __RequestVerificationToken: $('#frmBudgetAdditionalV2 input[name=__RequestVerificationToken]').val() },
        datatype: "json",
    ...
    oTableDetail = $('#dtDetail').DataTable({
        ...
        aoColumnDefs: [
            ...
            },
            {
                aTargets: [1],
                mRender: function (data, type, full) {
                    return '<div > <input type="text" class="form-control " id="txtPeriodeBudget' + full.intIndex + '" style="width:120px" class="txtPeriodeBudget" onchange="p_txtPeriodeBudget_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_PERIOD
                        + '" disabled> </div> </div>';
            ...
            },
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div > <input type="text" class="form-control " id="txtGroupAccount' + full.intIndex + '" style="width:120px" class="txtGroupAccount" onchange="p_txtGroupAccount_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_GROUP_ACCOUNT
                        + '" disabled> </div> </div>';
            ...
            },
            {
                aTargets: [3],
                mRender: function (data, type, full) {
                    return '<div > <input type="text" class="form-control " id="txtBudgetType' + full.intIndex + '" style="width:120px" class="txtBudgetType" onchange="p_txtBudgetType_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_BUDGET_TYPE
                        + '" disabled> </div> </div>';
...
function p_DataToUIBudget(objData) {
    ...
    //p_DataToUIDetailBudget(objData.XXSHP_KDS_M_BGT_DTL);
    p_DataToUIDetail(objData.XXSHP_KDS_M_BGTADD_DTL);
    p_DataToUIOther(objData.XXSHP_KDS_M_BGTADD_ADD);
    p_SetHiddenObject(objData);
...
function p_txtID_TextChanged() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/BudgetAdditionalV2/GetData",
        data: { txtID: $("#txtID").val(), __RequestVerificationToken: $('#frmBudgetAdditionalV2 input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_txtHeaderID_TextChanged() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/BudgetAdditionalV2/GetDataBudget",
        data: {
            ...
            __RequestVerificationToken: $('#frmBudgetAdditionalV2 input[name=__RequestVerificationToken]').val()
        },
        datatype: "json",
        success: function (retDat) {
...
function p_UploadBudgetDetail() {
    clsGlobal.getConfirmation("Upload Budget Detail?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Master/BudgetAdditionalV2/UploadBudgetDetail",
                processData: false,
                contentType: false,
                data: data,
                success: function (retDat) {
                    if (retDat.bitSuccess == true) {
                        if (retDat.objData != undefined) {
                            ...
                            p_DataToUI(retDat.objData);
                            p_CalculateTotal();
                            //p_DataToUIDetail(retDat.objData.XXSHP_KDS_M_BGTADD_DTL);
...
function p_DownloadBudgetDetail() {
    clsGlobal.getConfirmation("Download Template Budget Detail?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Master/BudgetAdditionalV2/DownloadBudgetDetail",
                datatype: "json",
...
function p_decAmountAdd_Changed(objCaller, intIndex) {
    ...
    for (i = 0; i < objData.XXSHP_KDS_M_BGTADD_DTL.length; i++) {
        ...
        if (objData.XXSHP_KDS_M_BGTADD_DTL[i].intIndex == intIndex) {
            // Ketemu, mulai dari sini:
            objData.XXSHP_KDS_M_BGTADD_DTL[i].DEC_AMOUNT = objCaller.value;
...
function p_DataToUIDetail(XXSHP_KDS_M_BGTADD_DTL) {
    ...
    for (var i = 0; i < XXSHP_KDS_M_BGTADD_DTL.length; i++) {
        ...
        //XXSHP_KDS_M_BGTADD_DTL[i].DEC_AMOUNT = clsGlobal.parseToRupiah(XXSHP_KDS_M_BGTADD_DTL[i].DEC_AMOUNT);
        //XXSHP_KDS_M_BGTADD_DTL[i].DEC_ENDINGBALANCE = clsGlobal.parseToRupiah(XXSHP_KDS_M_BGTADD_DTL[i].DEC_ENDINGBALANCE);
...
//function p_DataToUIDetailBudget(XXSHP_KDS_M_BGT_DTL) {
//    
//    oTableDetail.clear();
//    for (var i = 0; i < XXSHP_KDS_M_BGT_DTL.length; i++) {
//        XXSHP_KDS_M_BGT_DTL[i].intIndex = i;
//        XXSHP_KDS_M_BGT_DTL[i].DEC_AMOUNT = 0;        
...
function p_saveData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/BudgetAdditionalV2/SaveData",
        data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmBudgetAdditionalV2 input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_deleteData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/BudgetAdditionalV2/DeleteData",
        data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmBudgetAdditionalV2 input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
    ...
    oTableOther = $('#dtOther').DataTable({
        ...
        aoColumnDefs: [
            ...
            },
            {
                aTargets: [1],
                mRender: function (data, type, full) {
                    return '<div >  <input type="text" class="form-control" id="txtOtherPeriodeBudget' + full.intIndex + '" style="width:100px" class="txtOtherPeriodeBudget" onchange="p_txtOtherPeriodeBudget_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_PERIOD + '" >  </div>';
            ...
            },
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div >  <input type="text" class="form-control" id="txtOtherGroupAccount' + full.intIndex + '" style="width:100px" class="txtOtherGroupAccount" onchange="p_txtOtherGroupAccount_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_GROUP_ACCOUNT + '" >  </div>';
            ...
            },
            {
                aTargets: [3],
                mRender: function (data, type, full) {
                    return '<div >  <input type="text" class="form-control" id="txtOtherBudgetType' + full.intIndex + '" style="width:100px" class="txtOtherBudgetType" onchange="p_txtOtherBudgetType_Changed(this,' + full.intIndex + ')"  value="' + full.TXT_BUDGET_TYPE + '" >  </div>';
...
function p_settxtOtherUmBrand(txtValue1, txtValue2, txtValue3, txtValue4, txtValue5, txtValue6, txtValue7, txtValue8) {
    ...
    oTableOther.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        if (intRowIndex == intSelectedIndex) {
            ...
            //objDat.XXSHP_KDS_M_BGTADD_ADD[intRowIndex].DEC_ENDINGBALANCE = d.DEC_ENDINGBALANCE;
...
function p_AddOther() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/BudgetAdditionalV2/AddOther",
        data: { data: $("#txtHiddenObject").val(), __RequestVerificationToken: $('#frmBudgetAdditionalV2 input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_RefreshNumberOther() {
    ...
    oTableOther.rows().every(function (rowIdx, tableLoop, rowLoop) {
        ...
        //d.DEC_ENDINGBALANCE = objDat.XXSHP_KDS_M_BGTADD_ADD[intRowIndex].DEC_ENDINGBALANCE;
        intRowIndex++;
        this.invalidate(); // invalidate the data DataTables has cached for this row         
    });
