...
function setChooseLOV(txtValue) {
    ...
    switch (arr[0]) {
        ...
        //case "txtPeriod":
        //    $("#txtPeriod").val(arr[1]);
        //    p_txtPeriod_TextChanged();
        //    break;
            //case "txtSupplierName":
            //    $("#txtSupplierName").val(arr[1]);
            //    p_txtSupplierName_TextChanged();
            //    break;
            //case "intSupplierSiteCode":
            //    $("#intSupplierSiteCode").val(arr[1]);
...
function p_DataToUI(objData) {
    ...
    //$("#txtPeriod").val(objData.TXT_PERIOD);
    //p_txtPeriod_TextChanged();
    $("#fileUploadBudgetDetail").val(objData.empty);
    $("#txtID").val(clsGlobal.parseToInteger(objData.INT_BGT_HDR_ID));
...
function p_DataToUIDetail(XXSHP_KDS_M_BGT_DTL) {
    ...
    for (var i = 0; i < XXSHP_KDS_M_BGT_DTL.length; i++) {
        ...
        //XXSHP_KCO_M_BGT_DTL[i].DEC_OPENBALANCE = clsGlobal.parseToRupiah(XXSHP_KCO_M_BGT_DTL[i].DEC_OPENBALANCE);
...
function p_UploadBudgetDetail() {
    ...
    clsGlobal.getConfirmation("Upload Budget Detail?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Master/Budget/UploadBudgetDetail",
                processData: false,
                contentType: false,
                data: data,
                success: function (retDat) {
                    if (retDat.bitSuccess == true) {
                        if (retDat.objData != undefined) {
                            ...
                            //p_DataToUIDetail(retDat.objData.XXSHP_KCO_M_BGT_DTL);
                            //oTableDetail.page('last').draw(false);
                            //p_EnableControl(false);

                            //var objDat = p_GetHiddenObject();
                            //p_CalculateTotal();
                            //var index = objDat.XXSHP_KCO_M_BGT_DTL.length;
                            //var Total = 0;
...
function p_DownloadBudgetDetail() {
    clsGlobal.getConfirmation("Download Template Budget Detail?", function (result) {
        ...
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/Master/Budget/DownloadBudgetDetail",
                datatype: "json",
                data: {
...
function p_UIToData() {
    ...
    jsonData.INT_BGT_HDR_ID = clsGlobal.parseToInteger($("#txtID").val());
...
function p_EnableControl(bitApply) {
    ...
    if (bitApply == true) {
        ...
        $("#fileUploadBudgetDetail").attr("disabled", "true");
        ...
        $("#fileUploadBudgetDetail").removeAttr("disabled");
...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/InitiateData",
        data: { __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_PopulatePeriodAndSet(txtValue) {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/PopulatePeriod",
...
function p_PopulateBudgetTypeAndSet(txtValue) {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/PopulateBudgetType",
        data: { __RequestVerificationToken: $('#FormLogin input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
            if (retDat.bitSuccess == true) {
...
//    for (i = 0; i < objData.XXSHP_KCO_M_BGT_DTL.length; i++) {
//        // Cari Index-nya.
//        if (objData.XXSHP_KCO_M_BGT_DTL[i].intIndex == intIndex) {
//            // Ketemu, mulai dari sini:
//            objData.XXSHP_KCO_M_BGT_DTL[i].TXT_UMBRAND = objCaller.value;
//            break;
//        }
//    }
//    p_SetHiddenObject(objData);
...
function p_decOpenBalance_Changed(objCaller, intIndex) {
    ...
    //
    //var total = 0;
    //for (var i = 0; i < objData.XXSHP_KCO_M_BGT_DTL.length; i++) {
    //    //var tes = objData.XXSHP_KCO_M_BGT_DTL[i].DEC_OPENBALANCE;
    //    total = total + clsGlobal.parseToInteger(clsGlobal.parseToAngka(objData.XXSHP_KCO_M_BGT_DTL[i].DEC_OPENBALANCE));
...
function p_CalculateTotal() {
    ...
    var objData = JSON.parse(p_UIToData());
    for (i = 0; i < objData.XXSHP_KDS_M_BGT_DTL.length; i++) {
        decTotalBudget += objData.XXSHP_KDS_M_BGT_DTL[i].DEC_OPENBALANCE;
        decTotalTargetSales += objData.XXSHP_KDS_M_BGT_DTL[i].DEC_TARGETSALES;
    }
    if (decTotalTargetSales != 0) {
        decCostRatio = clsGlobal.parseToDecimal(decTotalBudget / decTotalTargetSales)*100;
    }
...
function p_settxtUmbrand(intBrandID) {
    ...
    $.ajax({
        ...
        data: {
            ...
            pTXT_PERIOD: TXT_PERIOD,
            __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val()
        },
        datatype: "json",
        success: function (retDat) {
            //
...
function p_txtUmbrand_Changed_LOV(objDat) {
    ...
    //buat ngevalidasi data nya jika ada yang sama
    var isValidData = true;
    for (i = 0; i < index; i++) {
        var UmbrandName = objData.XXSHP_KDS_M_BGT_DTL[i].TXT_UMBRAND;
        var SubUmbrandName = objData.XXSHP_KDS_M_BGT_DTL[i].TXT_SUBUMBRAND;
        var BrandName = objData.XXSHP_KDS_M_BGT_DTL[i].TXT_BRAND;
        var SubBrandName = objData.XXSHP_KDS_M_BGT_DTL[i].TXT_SUBBRAND;

        if (objData.XXSHP_KDS_M_BGT_DTL[i].TXT_UMBRAND == objDat.txtumBrandName &&
            objData.XXSHP_KDS_M_BGT_DTL[i].TXT_SUBUMBRAND == objDat.txtSubUmbrand &&
            objData.XXSHP_KDS_M_BGT_DTL[i].TXT_BRAND == objDat.txtBrandName &&
            objData.XXSHP_KDS_M_BGT_DTL[i].TXT_SUBBRAND == objDat.DescSubBrand) {
            clsGlobal.showAlert("Umbrand : "+ UmbrandName +" | SubUmbrand : "+ SubUmbrandName+ " | Brand : " + BrandName+ " | SubBrand : "+ SubBrandName +" | Sudah dipilih!");
            isValidData = false;
            break;
        } else {
...
//function p_txtPeriod_TextChanged() {
//    clsGlobal.showLoading();
//    $.ajax({
//        type: "POST",
//        url: "/Master/Budget/GetDataPeriod",
//        data: { txtPeriod: $("#txtPeriod").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
//        datatype: "json",
//        success: function (retDat) {
...
function p_txtSupplierID_TextChanged() {
    ...
    $.ajax({
        ...
        data: { txtID: $("#txtGroupAccount").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_txtSupplierName_TextChanged() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/GetDataVendor",
        data: { VENDOR_ID: $("#txtSupplierName").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_txtSupplierSiteCode_TextChanged() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/GetDataVendorSite",
        data: { VENDOR_SITE_ID: $("#intSupplierSiteCode").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
//function p_txtBudgetType_TextChanged() {
//    clsGlobal.showLoading();
//    $.ajax({
//        type: "POST",
//        url: "/Master/Budget/GetDataBudgetType",
//        data: { txtBudgetType: $("#txtBudgetType").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
//        datatype: "json",
//        success: function (retDat) {

//            if (retDat.bitSuccess == true) {
//                if (retDat.objData != undefined) {
//                    $("#txtBudgetType").val(retDat.objData.DESCRIPTION);
//                } else {
//                    p_showBlank();
//                }
//            } else {
...
function p_AddDetail() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/AddRow",
        data: { data: $("#txtHiddenObject").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_saveData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Master/Budget/SaveData",
        data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmBudget input[name=__RequestVerificationToken]').val() },
        datatype: "json",
        success: function (retDat) {
...
function p_btnUmbrandLOVID(objCaller, intIndex) {
    ...
    try {
        ...
        if (TXT_PERIOD == "" || TXT_PERIOD == null) {
            ...
            LOV = clsGlobal.generateLOV(MODULE_BRAND_BRANDBUDGET, "txtUmBrand", TXT_PERIOD);
        }
    } catch (ex) {
        clsGlobal.showAlert(ex);
    }
