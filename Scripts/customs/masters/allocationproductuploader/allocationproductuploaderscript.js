...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/AllocationProductUploader/InitiateData",
...
function p_txtDocNo_TextChanged(ID) {
    ...
    $.ajax({
        type: "POST",
        url: "/AllocationProductUploader/GetData", 
...
function p_processData() { 
    ...
    $.ajax({
        type: "POST",
        url: "/AllocationProductUploader/ProcessData",
...
function p_DataToUIDetail(XXSHP_KDS_M_ALOPRO_UPL) {
    
    oTableDetail.clear();
    for (var i = 0; i < XXSHP_KDS_M_ALOPRO_UPL.length; i++) {
        XXSHP_KDS_M_ALOPRO_UPL[i].intIndex = i;
        oTableDetail.row.add(XXSHP_KDS_M_ALOPRO_UPL[i]);
    }
    oTableDetail.draw(false);

    var objDat = p_GetHiddenObject();
    objDat = XXSHP_KDS_M_ALOPRO_UPL;
    ...
    oTableDetail = $('#dtDetail').DataTable({
        ...
        aoColumnDefs: [
            ...
            },
            {
                aTargets: [1],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.INT_ALOPRO_HDR_ID) + ' </div>';
                }
            },
            {
                aTargets: [2],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.TXT_PERIOD) + ' </div>';
                }
            },
            {
                aTargets: [3],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.TXT_GROUP_ACCOUNT) + ' </div>';
                }
            },
            {
                aTargets: [4],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.TXT_SUPPLIER_CODE) + ' </div>';
                }
            },
            {
                aTargets: [5],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.TXT_SUPPLIER_NAME) + ' </div>';
                }
            },
            {
                aTargets: [6],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.TXT_LOB) + ' </div>';
                }
            },
            {
                aTargets: [7],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailParentDocNoValue"> ' + clsGlobal.parseToString(full.TXT_UMBRAND) + ' </div>';
                }
            },
            {
                aTargets: [4],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailReffDocNoValue"> ' + clsGlobal.parseToString(full.TXT_SUBUMBRAND) + ' </div>';
                }
            },
            {
                aTargets: [8],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailReffDocNoValue"> ' + clsGlobal.parseToString(full.TXT_BRAND) + ' </div>';
                }
            },
            {
                aTargets: [9],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailOutletValue"> ' + clsGlobal.parseToString(full.TXT_SUBBRAND) + ' </div>';
                }
            },
            {
                aTargets: [10],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailPaymentValue"> ' + clsGlobal.parseToString(full.TXT_ITEMCODE) + ' </div>';
                }
            },
            {
                aTargets: [11],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailRemarkValue"> ' + clsGlobal.parseToString(full.TXT_ITEMDESC) + ' </div>';
                }
            },
            {
                aTargets: [12],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailMonthlyPromoValue"> ' + clsGlobal.parseToString(full.DEC_ALOKASI) + ' </div>';
                }
            }, 
            {
                aTargets: [13],
...
function p_Upload() {
    clsGlobal.getConfirmation("Upload Allocation Product?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/AllocationProductUploader/Upload",
                processData: false,
                contentType: false,
                data: data,
                success: function (retDat) {
...
function p_CheckCanProcess(XXSHP_KDS_M_ALOPRO_UPL) {
    $("#btnProcess").hide();
    
    var bitFlag = true;
    for (var i = 0; i < XXSHP_KDS_M_ALOPRO_UPL.length; i++) {
        if (XXSHP_KDS_M_ALOPRO_UPL[i].TXT_STATUS == "E" || XXSHP_KDS_M_ALOPRO_UPL[i].TXT_STATUS == "S") {
            bitFlag = false;
        }       
    } 
    if (bitFlag == true) {
...
function p_Download() {
    clsGlobal.getConfirmation("Download Template Upload Allocation Product?", function (result) {
        if (result == true) {
            clsGlobal.showLoading();
            $.ajax({
                type: "POST",
                url: "/AllocationProductUploader/Download",
...
function p_DownloadResult() {
    clsGlobal.getConfirmation("Download Result?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/AllocationProductUploader/DownloadResult",
                data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmIndexUploader input[name=__RequestVerificationToken]').val() },
                datatype: "json",
                success: function (url) {
                    window.location = url;
