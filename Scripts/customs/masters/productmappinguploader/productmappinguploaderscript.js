...
function p_initiateData() {
    ...
    $.ajax({
        type: "POST",
        url: "/ProductMappingUploader/InitiateData",
...
function p_txtDocNo_TextChanged(ID) {
    ...
    $.ajax({
        type: "POST",
        url: "/ProductMappingUploader/GetData", 
...
function p_processData() { 
    ...
    $.ajax({
        type: "POST",
        url: "/ProductMappingUploader/ProcessData",
...
function p_DataToUIDetail(XXSHP_KDS_M_PROMAP_UPL) {
    
    oTableDetail.clear();
    for (var i = 0; i < XXSHP_KDS_M_PROMAP_UPL.length; i++) {
        XXSHP_KDS_M_PROMAP_UPL[i].intIndex = i;
        oTableDetail.row.add(XXSHP_KDS_M_PROMAP_UPL[i]);
    }
    oTableDetail.draw(false);

    var objDat = p_GetHiddenObject();
    objDat = XXSHP_KDS_M_PROMAP_UPL;
    ...
    oTableDetail = $('#dtDetail').DataTable({
        ...
        aoColumnDefs: [
            ...
            },
            {
                aTargets: [1],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailDocNoValue"> ' + clsGlobal.parseToString(full.INT_PROMAP_HDR_ID) + ' </div>';
                }
            },
            {
                aTargets: [2],
            ...
            },
            {
                aTargets: [7],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailPaymentValue"> ' + clsGlobal.parseToString(full.TXT_ITEMCODE_H) + ' </div>';
                }
            },
            {
                aTargets: [8],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailRemarkValue"> ' + clsGlobal.parseToString(full.TXT_ITEMDESC_H) + ' </div>';
                }
            },
            {
                aTargets: [9],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailMonthlyPromoValue"> ' + clsGlobal.parseToString(full.TXT_ITEMCODE_D) + ' </div>';
                }
            },
            {
                aTargets: [10],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailNationalValue"> ' + clsGlobal.parseToString(full.TXT_ITEMDESC_D) + ' </div>';
                }
            },
            {
                aTargets: [11],
                mRender: function (data, type, full) {
                    return '<div id="lblDetailActivityValue"> ' + clsGlobal.parseToString(full.TXT_STATUS_DATA) + ' </div>';
                }
            }, 
            {
                aTargets: [12],
...
function p_Upload() {
    clsGlobal.getConfirmation("Upload Product Mapping?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/ProductMappingUploader/Upload",
                processData: false,
                contentType: false,
                data: data,
                success: function (retDat) {
...
function p_CheckCanProcess(XXSHP_KDS_M_PROMAP_UPL) {
    $("#btnProcess").hide();
    
    var bitFlag = true;
    for (var i = 0; i < XXSHP_KDS_M_PROMAP_UPL.length; i++) {
        if (XXSHP_KDS_M_PROMAP_UPL[i].TXT_STATUS == "E" || XXSHP_KDS_M_PROMAP_UPL[i].TXT_STATUS == "S") {
            bitFlag = false;
        }       
    } 
    if (bitFlag == true) {
...
function p_Download() {
    clsGlobal.getConfirmation("Download Template Upload Product Mapping?", function (result) {
        if (result == true) {
            clsGlobal.showLoading();
            $.ajax({
                type: "POST",
                url: "/ProductMappingUploader/Download",
...
function p_DownloadResult() {
    clsGlobal.getConfirmation("Download Result?", function (result) {
        if (result == true) {
            ...
            $.ajax({
                type: "POST",
                url: "/ProductMappingUploader/DownloadResult",
                data: { data: $("#txtHiddenObject").val(), txtGUID: $("#txtGUID").val(), __RequestVerificationToken: $('#frmIndexUploader input[name=__RequestVerificationToken]').val() },
                datatype: "json",
                success: function (url) {
                    window.location = url;
